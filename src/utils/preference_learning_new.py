...
def create_preference_dataset_for_eval(preference_path, budget):
    flatten_encoded = {
        key: np.array(value).flatten()
        for key, value in load_encoded(preference_path).items()
    }
    ...
    preferences = load_preferences(path=preference_path)
    dataset = {}
    for label, set in {"train": train, "test": test}.items():
        dataset[label] = [
            [
                flatten_encoded[str(tuple[0])],
                flatten_encoded[str(tuple[1])],
                tuple[2],
            ]
            for tuple in preferences.to_numpy()
            if str(tuple[0]) in set and str(tuple[1]) in set
        ]
        ConfDict(
            {
                f"X_{label}": np.array(
                    [[tuple[0], tuple[1]] for tuple in dataset[label]]
                ),
                f"Y_{label}": np.array([tuple[2] for tuple in dataset[label]]),
                f"preferences_{label}": [
                    tuple
                    for tuple in preferences[["pair_0", "pair_1"]].to_numpy()
                    if str(tuple[0]) in set and str(tuple[1]) in set
                ],
                "iteration": 0,
...
def compute_raw_results(config_dict, result_dict, seed):
    fate = MyRankSVM(**config_dict, random_state=seed)
    fate.fit(ConfDict()["X_train"].copy(), ConfDict()["Y_train"])

    raw_results = pd.DataFrame(
        {
            "pair_0": [pref[0] for pref in ConfDict()["preferences_test"]],
            "pair_0_score": [
                elem
                for list in fate.predict_scores(
                    np.array([[elem[0]] for elem in ConfDict()["X_test"]])
                )
                for elem in list
            ],
            "pair_1": [pref[1] for pref in ConfDict()["preferences_test"]],
            "pair_1_score": [
                elem
                for list in fate.predict_scores(
                    np.array([[elem[1]] for elem in ConfDict()["X_test"]])
                )
                for elem in list
            ],
            "y_true": ConfDict()["Y_test"],
            "y_pred": fate.predict(ConfDict()["X_test"].copy()),
        }
    )

    # feat_dict[mode] = fate.features_expl_
    result_dict["accuracy"] = accuracy_score(
        raw_results["y_true"], raw_results["y_pred"]
    )
    raw_results.to_csv(
        os.path.join(
            make_dir(
                os.path.join(ConfDict()["output_folder"], f"""{ConfDict()["budget"]}""")
            ),
...
def objective(config: Configuration, seed: int = 0) -> float:
    ...
    try:
        compute_raw_results(config_dict, result_dict, seed)
